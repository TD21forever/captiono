...
- Compact annotation editors submit with plain `Enter`, keep `Shift+Enter` for a newline, and must not submit while a Chinese/Japanese/Korean IME composition is active.
- Manual transcript browsing resumes playback-follow automatically only after a full idle window: scrolling, selection, open annotation/search/settings UI, focus inside the panel, or merely keeping the pointer over Captiono all postpone the return. After eight seconds with none of those signals, restore `following` and place the current sentence near the top third again.
...
- On Bilibili, keep the native uploader/profile status card above Captiono and place Captiono immediately before the native `.danmaku-box` / `#danmukuBox` module in that module's existing parent. Never hoist the danmaku anchor to a `.right-container` direct child: doing so moves the profile card below the 360–560px panel and makes it appear missing. Only mount when the verified video layout, player, paired right column, danmaku parent, and player/non-overlap geometry are all safe.
...
- The collapsed launcher must not translate on hover, because moving its hit target can create pointer-boundary oscillation. Stop click and press/release events at the Shadow Root so Bilibili's delegated page handlers cannot consume panel controls, but do not suppress hover-boundary events such as `mouseover`/`pointerover`; suppressing them can leave host-page hover state unbalanced.
...
- Embedded density responds to the module's own sidebar width through container queries. Bilibili keeps a 64px launcher and a bounded expanded module that preserves useful transcript context without consuming the native sidebar's remaining page content.
- Automatic acquisition may retry for DOM/media identity changes, track insertions, tab visibility recovery, or an explicit learner refresh. A definitive `empty`/`unavailable` result must never start an unbounded timer loop or alternate the full panel between loading and empty states.
...
- Coordinate-based transcript affordances (phrase tooltip, selection toolbar, and compact annotation editor) use absolute positioning inside the stage and convert DOM viewport rectangles into stage-local coordinates. Never combine viewport coordinates with fixed positioning beneath a `container-type` or containment ancestor, because the host becomes the fixed containing block and offsets the affordance twice.
...
- The injected module uses a closed Shadow Root and never stores panel state in page `localStorage` or `sessionStorage`. High-frequency caption/media state stays on the in-page bridge instead of waking the MV3 service worker every 500ms.
- `npm run build:extension` must rebuild both `dist/extension/` and the versioned release ZIP, then verify that every archived file is byte-identical to the unpacked build. Never publish a ZIP left over from an earlier source revision.
...
- The bilingual README may use real product screenshots supplied by the user; keep the main overview image and add focused workflow screenshots as repository assets with stable relative paths.
